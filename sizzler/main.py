"""Main training script for Sizzler Seq-GAN."""
...
from sizzler.config import DEFAULT_DATA_PATH, DEFAULT_OUTPUT_PATH


# Arguments
parser = argparse.ArgumentParser(description='Sizzler SeqGAN for PLC Fuzzing')
parser.add_argument('--hpc', action='store_true', default=False,
                    help='set to hpc mode')
parser.add_argument('--data_path', type=str, default=str(DEFAULT_DATA_PATH), metavar='PATH',
                    help=f'data path to save files (default: {DEFAULT_DATA_PATH})')
...
parser.add_argument('--n_samples', type=int, default=6400, metavar='N',
                    help='number of samples generated per time (default: 6400)')
...
def train_generator_PG(gen, dis, rollout, pg_loss, optimizer, epochs, args):
    ...
    for epoch in range(epochs):
        ...
        samples = gen.sample(args.batch_size, g_seq_len)
...
def train_discriminator(dis, gen, criterion, optimizer, epochs,
                        ...
                        args, positive_file, negative_file):
    """Train discriminator."""
...
def main():
    ...
    positive_file = str(data_path / 'data1.data')
    negative_file = str(data_path / 'gene.data')
    ...
    discriminator = Discriminator(d_num_class, args.vocab_size, d_embed_dim,
                                   d_filter_sizes, d_num_filters, d_dropout_prob)
    ...
    # Generate toy data using target LSTM (if needed)
    print('#####################################################')
    ...
    for i in range(args.d_pretrain_steps):
        ...
        train_discriminator(discriminator, generator, nll_loss,
                            dis_optimizer, args.dk_epochs, dis_adversarial_train_loss,
                            dis_adversarial_train_acc, args, positive_file, negative_file)
        generate_samples(generator, args.batch_size, args.n_samples, negative_file)
    ...
    for i in range(args.rounds):
        ...
        # Save model checkpoint every 10 rounds or at the end
        if i % 10 == 0 or i == args.rounds - 1:
            model_path = data_path / f'generator_round_{i}.pt'
            torch.save(generator.state_dict(), str(model_path))
            print(f"Saved model checkpoint to {model_path}")

    # Save experiment data
    experiment_file = data_path / 'experiment.pkl'
    ...
    print(f"Saved experiment results to {experiment_file}")
